...
if(canSense==1){ // Using timer 0 to continuously update the sensor array
	...
	if (sensors[SenIRChar]==248 || sensors[SenIRChar]==244 || sensors[SenIRChar]==242 || 
		sensors[SenIRChar]==252 || sensors[SenIRChar]==250 || sensors[SenIRChar]==246 ||
		sensors[SenIRChar]==254 || sensors[SenIRChar]==240)
		robotState=3; // Docking station signal detected
...
if(sensors[SenBumpDrop]<=4) {
	if(robotState==3)
		dock();
	else if (robotState==0)// no wall, drive straight
		driveStraight();
...
if(sensors[SenIRChar]==252 || sensors[SenIRChar]==254){//redGreen
//	if(slowTheFuckDown)
//		home();
	driveStraightHome();
}
else if(sensors[SenIRChar]==248 || sensors[SenIRChar]==250)//red
	faceHome(1);
else if(sensors[SenIRChar]==244 || sensors[SenIRChar]==246)//green
	faceHome(0);
}
