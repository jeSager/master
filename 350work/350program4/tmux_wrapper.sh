...
if [[ -z $TMUX ]]; then
  ...
  tmux send-keys -t ${SESSION} 'cd $(ls -d mydirectory/*) && vim .' C-m
  tmux split -h
...
#  tmux set -g status off
  ...
  while [[ $RUN == 1 ]]; do
    ...
    cd $OUTPUT_DIR
    echo "Creating an array of the output file names"
    ...
    vim -M "+noremap <cr> :cq!<cr>" "+noremap q :qa!<cr>" "+set nonumber" "+set wrap linebreak nolist" ${OUTFILES[@]}
    RUN=$?
    cd ..
    clear
  done
